...
scriptCmd opts = do
    ...
    local (over globalOptsL modifyGO) $ case soCompile opts of
  ...
  longWay file scriptDir = do
    withLoadConfig $ withActualBuildConfig $ withDefaultBuildConfigAndLock $ \lk -> do
      ...
      stackYaml <- view $ globalOptsL.to globalStackYaml
      case stackYaml of
